...
{$include vfmod2const.inc}
{$include vfmod2types.inc}
...
function FMOD2_ErrorString( ErrCode: FMOD_RESULT ): AnsiString;

var
  FMOD2 : TLibrary = nil;

function LoadFMOD2( const aPath : AnsiString = FMOD2DefaultPath ) : Boolean;
...
function LoadFMOD2 ( const aPath : AnsiString ) : Boolean;
  function GetSymbol( const aSymbol : AnsiString ) : Pointer;
  begin
    GetSymbol := FMOD2.Get( aSymbol );
    if GetSymbol = nil then
      raise ELibraryError.Create( 'FMOD : Symbol "'+aSymbol+'" not found!' );
  end;
begin
  if FMOD2 <> nil then Exit( True );

  Saved8087CW := Default8087CW;
  Set8087CW($133f); { Disable all fpu exceptions }

  FMOD2 := TLibrary.Load( aPath );
  if FMOD2 = nil then Exit( False );
